...
- (void)viewDidLoad {
    ...
    _topView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width ,headerHeight)] ;
    _topView.backgroundColor = [UIColor redColor] ;
    
    //展示用的topView需要添加在tableView上来进行联动
    [self.tableView addSubview:_topView] ;
    [self.tableView bringSubviewToFront:_topView] ;

    _footView = [[UIView alloc] initWithFrame:CGRectMake(0, [UIScreen mainScreen].bounds.size.height - tabBarHeight - footerHeight, [UIScreen mainScreen].bounds.size.width, footerHeight)] ;
    ...
    [_footView addSubview:smallItem1] ;
    ...
    [_footView addSubview:smallItem2] ;
    ...
    [_footView addSubview:smallItem3] ;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (tableViewoffsetY >= 0) {
        //上滑,topView随tableView滑动
        _topView.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, headerHeight) ;
        //底部模块处理
        if(tableViewoffsetY <= footerHeight/2) {
            ...
            _footView.frame = CGRectMake(0, [UIScreen mainScreen].bounds.size.height-tabBarHeight-footerHeight/2, [UIScreen mainScreen].bounds.size.width, footerHeight/2) ;
        ...
        //下滑，topView保持不动
        //下滑时，tableView的frame.origin.y不会变化，但是tableView的bounds.origin.y会变化，所以tableView上所有子页面相对于tableView父页面的位置就会发生变化，此处就是_topView会随着_tableView的滑动一起移动。通过改变_topView的frame.origin.y来固定_topView在页面上
        self.topView.frame = CGRectMake(0, tableViewoffsetY, [UIScreen mainScreen].bounds.size.width, headerHeight) ;
        //底部模块保持不变
        _footView.frame = CGRectMake(0, [UIScreen mainScreen].bounds.size.height - tabBarHeight - footerHeight, [UIScreen mainScreen].bounds.size.width, footerHeight) ;
...
- (void)cellDeletebuttonClickedWithCell:(UITableViewCell *)cell {
    ...
    [_tableView bringSubviewToFront:_topView] ; //防止topView被tableView的headerView覆盖
}
@end
